...
//	    	gl.glDisable(GL.GL_LINE_SMOOTH);
	...
	public void draw(){
		try {
			...
			e.printStackTrace();
			System.out.println("Modifica fatta un MTApplication.java nel metodo draw");
		}
...
//		Iscene stackScene = sceneStack.pollFirst();
	...
	private boolean initiateSceneChange(Iscene oldScene, Iscene newScene){
		...
		if (!sceneChangeLocked && ActiveCursorPool.getInstance().getActiveCursorCount()==0){
			...
			//questa è una modifica che sto provando io
			InputCursor cursor[] = ActiveCursorPool.getInstance().getActiveCursors();
			for ( int i = 0 ; i < cursor.length ; i++ ){
				InputCursor c = cursor[i];
				System.out.println("Sto rimuovendo il cursore : " + c.getId());
				ActiveCursorPool.getInstance().removeCursor(c.getId());
			}
